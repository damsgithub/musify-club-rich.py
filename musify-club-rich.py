...
------------------------------------------------------------------------------------------------------------------
user@computer:/tmp$ %s [-p /path] %s/release/the-6-cello-suites-cd1-1994-630746
...
user@computer:/tmp$ %s [-p /path] %s/artist/johann-sebastian-bach-7110
** We will try to use 3 simultaneous downloads, progress will be shown **
** after each completed file but not necessarily in album's order. **
...
def download_cover(page_content, url, debug, socks_proxy, socks_port, timeout, task_id, event):
    ...
    cover_url_re = re.compile('<link href="(.+?)" rel="image_src"(?:\s)*/?>')
...
def download_song(num_and_url, debug, socks_proxy, socks_port, timeout, task_id: TaskID, event) -> None:
    ...
    while True:  # continue until we have the song or the user interrupts it
        try:
            ...
            if debug:
                color_message("%s: downloading song from %s" % (process_id, file_url), debug_color)
            ...
            if debug:
                color_message(
                    ...
                    % (process_id, file_url, file_name, str(e)),
                    warning_color,
                )
            pause_between_retries()
            pass
...
def download_album(url, base_path, debug, socks_proxy, socks_port, timeout, nb_conn, event, alt_dir_fmt):
    ...
    for div_link in page_soup.find_all("div", title=re.compile("^Слушать.*")):
        # search track number and link
        # <div class="playlist__control play" data-position="1" data-title="Rammstein - Engel" data-url="/track/play/10442917/rammstein-engel.mp3" id="play_10442917" title="Слушать Rammstein - Engel">
        tracknum = 0
        link_href = ""
        ...
        tracknum_infos = tracknum_infos_re.search(str(div_link))
        ...
        if tracknum_infos:
            ...
            if debug:
                color_message("** Unable to get track number for %s **" % (link_href), warning_color)
    ...
    deleted_track_re = re.compile(
        r'<div class="playlist__position">\r?\n?'
        '(?:\r?\n?)?'
        '(?:\s)*(\d+)\r?\n?'
        '(?:\r?\n?)?'
        '(?:\s)*</div>(?:\s)*\r?\n?'
        '(?:\r?\n?)?'
        '(?:\s)*<div class="playlist__details">(?:\s)*'
        '(?:\r?\n?)?'
        '(?:\s)*<div class="playlist__heading">(?:\s)*'
        '(?:\r?\n?)?'        
...
def download_artist(url, base_path, debug, socks_proxy, socks_port, timeout, nb_conn, event):
    ...
    for link in page_soup.find_all("a", href=True):
        if re.search(r"/release/.*", link["href"]):
